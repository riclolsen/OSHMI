...
GtkWidget *color_list, *script_list;
...
void isac_menu_init(GtkTable *table, GObject *spw)
{
	/***************************************************
	ISAC FEATURES:
	...
	definied.
	***************************************************/
	GtkWidget *frm, *nb, *frame_hb, *frame_vb, *frame_hb_butt, *apply_bttn, *ok_bttn, *cancel_bttn, *frame_nb;
	/*ISAC frame
	...
	*/
	...
	frame_hb_butt = gtk_hbox_new(FALSE, 3);
...
static void isac_notebook_new(SPWidget *spw)
{
	...
	**************************************************/
	...
	nb = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_nb"));
	GtkWidget *frame_nb = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_nb_frame"));
	...
	else if (SP_IS_RECT(item))
	{
		...
		gint prev_nbtype = prefs->getInt(prefs_path + "nbtype", 0);
		gint prev_nbtypepage = prefs->getInt(prefs_path + "nbtypepage", 1);
	...
	g_signal_connect(GTK_OBJECT(nb), "switch-page",
		G_CALLBACK(isac_ontabchanged), spw);
...
static void isac_open_combo_changed(GtkWidget *widget, SPWidget *spw)
{
	...
	*****************************/
	...
	wg_xpos = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_open_xpos"));

	wg_ypos = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_open_ypos"));

	wg_width = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_open_width"));

	wg_height = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_open_height"));

	wg_type = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_open_type"));
...
void isac_update_syntax(GtkWidget *widget, SPWidget *spw)
{
	...
	*****************************/
	...
	gchar *isac_syntax = "";
	label_entry = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "label"));
...
void isac_json_parse_syntax(const gchar *syntax, SPWidget *spw){
	...
	for (size_t i = 0; i<nCount; i++)
	{
		...
		if (strcmp(attr, "bar") == 0)
		{
			isac_bar_assign_syntax(token, spw);
			if (strlen(actpage) == 0)
				actpage = "Bar ";
		}
		else if (strcmp(attr, "clone") == 0)
		{
...
static void isac_setActiveTab(gchar *lbl, SPWidget *spw) // isac_setActiveTab("Color ", spw);
{
	...
	//    active/in-focus. 
	//------------------------------------------------
	GtkNotebook *nb = GTK_NOTEBOOK(g_object_get_data(GTK_OBJECT(spw), "isac_nb"));
...
void isac_change_mode(SPWidget *spw)
{
	...
	//------------------------------------------------
	GtkWidget *nb = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_nb"));
...
static gboolean script_onchange_save(GtkWidget *widget, GdkEventKey *event, SPWidget *spw)
{
	...
	wg_script = GTK_TEXT_VIEW(g_object_get_data(GTK_OBJECT(spw), "isac_script_code"));
	...
	if (gtk_tree_selection_get_selected(GTK_TREE_SELECTION(selection), &model, &iter)) {
		if (strlen(script) == 0) {
			gtk_list_store_set(store, &iter, 1, PANGO_UNDERLINE_NONE, 2, "", -1);
		}
		else {
			gtk_list_store_set(store, &iter, 1, PANGO_UNDERLINE_SINGLE, 2, script, -1);
		}
	}
	return 0;
}
void script_onchange(GtkWidget *widget, SPWidget *spw)
{
	...
	if (gtk_tree_selection_get_selected(GTK_TREE_SELECTION(widget), &model, &iter)) {
		...
		gtk_tree_model_get(model, &iter, 2, &scr, -1);
		wg_script = GTK_TEXT_VIEW(g_object_get_data(GTK_OBJECT(spw), "isac_script_code"));
		buffer = gtk_text_view_get_buffer(wg_script);
...
static gboolean isac_ontabchanged(GtkWidget *widget, gpointer page, guint tabindex, SPWidget *spw)
{
	//------------------------------------------------
	//    ISAC Features:
...
static void isac_color_list_onselected(GtkWidget *widget, SPWidget *spw)
{
	...
	bttn_picker = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_color_picker_bttn"));
	GtkWidget *color_find_tag = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_color_find_tag"));
...
void color_delete_item(GtkWidget *button, SPWidget *spw)
{
	...
	bttn_off = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_color_picker_bttn"));
...
void findtag_button_press_callback(GtkButton *button, gpointer user_data)
{
	...
	gtk_window_set_transient_for(GTK_WINDOW(findtag_window), GTK_WINDOW(dlg));
...
GtkWidget *isac_menu_bar_init(GObject *spw)
{
	/*****************************
	ISAC Features:
...
GtkWidget *isac_menu_clone_init(GtkObject *spw)
{
	...
	Setting up all the related entry fields.
	*****************************/
	GtkWidget *vb, *tb, *tb1, *tbscroll, *tbspare, *lbl1, *lbl2, *lbl3, *lblx, *tfx, *scroll;

	GtkTooltips *tt = gtk_tooltips_new();

	vb = gtk_vbox_new(FALSE, 4);
	tb = gtk_table_new(4, 3, FALSE);  //(rows, cols, homogeneous) dennis
	tb1 = gtk_table_new(3, 2, FALSE);
...
GtkWidget *isac_menu_get_init(GtkObject *spw)
{
	...
	*****************************/
	...
	cb1 = gtk_combo_box_new_text();
	gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb1), "Right");
	gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb1), "Left");
	// gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb1 ), "Center");
	gtk_combo_box_set_active(GTK_COMBO_BOX(cb1), 0);
	...
	gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb2), "Good");
	gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb2), "Live");
...
GtkWidget *isac_menu_open_init(GtkObject *spw)
{
	...
	*****************************/
	...
	gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb3), "URL");
	gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb3), "Tag");
	...
	gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb4), "Current window");
	gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb4), "New window");
...
GtkWidget *isac_menu_script_init(GtkObject *spw)
{
	...
	*****************************/
	GtkWidget *hbox, *vbox, *scroll, *tf, *textframe, *tbscroll, *tbspare, *tscroll;
	GtkTreeSelection *selection;
	GtkListStore *store;
	...
	GtkTreePath *path = gtk_tree_path_new_first();
	GtkTextBuffer *buffer;
	char *event[] = { "mouseup", "mousedown", "mouseover", "mouseout", "mousemove" };
	...
	for (int i = 0; i< 5; i++) // append the tab events "mouseup" "mousedown" "mouseover" "mouseout" "mousemove"
	{
		gtk_list_store_append(store, &iter);
		// append to column 0 each tab event & append to column 2 with null string
		gtk_list_store_set(store, &iter, 0, event[i], 2, "", -1);
	}
	...
	gtk_table_attach_defaults(GTK_TABLE(tbscroll), textframe, 0, 8, 0, 6);
...
GtkWidget *isac_menu_set_init(GtkObject *spw)
{
	...
	*****************************/
	...
	cb2 = gtk_combo_box_new_text();
	gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb2), "Data");
	gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb2), "Variable");
	gtk_combo_box_set_active(GTK_COMBO_BOX(cb2), 0);
...
GtkWidget *isac_menu_zoom_init(GtkObject *spw)
{
	...
	*****************************/
	...
	cb1 = gtk_combo_box_new_text();
	gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb1), "None");
	gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb1), "Center");
	gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb1), "Left");
	gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb1), "Right");
	gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb1), "Top");
	gtk_combo_box_append_text(GTK_COMBO_BOX_TEXT(cb1), "Bottom");
	gtk_combo_box_set_active(GTK_COMBO_BOX(cb1), 0);
...
static gboolean isac_validate_bar(SPWidget *spw)
{
	...
	wg_tag = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_bar_tag"));
	tag = gtk_entry_get_text(GTK_ENTRY(wg_tag));

	wg_min = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_bar_min"));
	min = gtk_entry_get_text(GTK_ENTRY(wg_min));

	wg_max = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_bar_max"));
...
static gboolean isac_validate_color(SPWidget *spw)
{
	...
	color_pick = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_color_picker_bttn"));
	GtkWidget *color_find_tag = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_color_find_tag"));
...
static gboolean isac_validate_function(SPWidget *spw)
{
	...
	wg_name = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_func_name"));
...
static gboolean isac_validate_get(SPWidget *spw)
{
	...
	wg_tag = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_get_tag"));
	tag = gtk_entry_get_text(GTK_ENTRY(wg_tag));
...
static gboolean isac_validate_opacity(SPWidget *spw)
{
	...
	wg_tag = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_opac_tag"));
	tag = gtk_entry_get_text(GTK_ENTRY(wg_tag));

	wg_min = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_opac_min"));
	min = gtk_entry_get_text(GTK_ENTRY(wg_min));

	wg_max = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_opac_max"));
	max = gtk_entry_get_text(GTK_ENTRY(wg_max));
...
static gboolean isac_validate_open(SPWidget *spw)
{
	...
	wg_source = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_open_source"));
...
static gboolean isac_validate_popup(SPWidget *spw)
{
	...
	wg_source = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_popup_source"));
	source = gtk_entry_get_text(GTK_ENTRY(wg_source));

	wg_xpos = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_popup_xpos"));
	xpos = gtk_entry_get_text(GTK_ENTRY(wg_xpos));

	wg_ypos = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_popup_ypos"));
	ypos = gtk_entry_get_text(GTK_ENTRY(wg_ypos));

	wg_width = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_popup_width"));
	width = gtk_entry_get_text(GTK_ENTRY(wg_width));

	wg_height = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_popup_height"));
	height = gtk_entry_get_text(GTK_ENTRY(wg_height));
...
static gboolean isac_validate_rotate(SPWidget *spw)
{
	...
	wg_tag = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_rot_tag"));
	tag = gtk_entry_get_text(GTK_ENTRY(wg_tag));

	wg_min = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_rot_min"));
	min = gtk_entry_get_text(GTK_ENTRY(wg_min));

	wg_max = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_rot_max"));
	max = gtk_entry_get_text(GTK_ENTRY(wg_max));
...
static gboolean isac_validate_set(SPWidget *spw)
{
	...
	wg_tag = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_set_tag"));
	tag = gtk_entry_get_text(GTK_ENTRY(wg_tag));
...
static gboolean isac_validate_slider(SPWidget *spw)
{
	...
	wg_tag = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_slider_tag"));
	tag = gtk_entry_get_text(GTK_ENTRY(wg_tag));

	wg_min = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_slider_min"));
	min = gtk_entry_get_text(GTK_ENTRY(wg_min));

	wg_max = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_slider_max"));
	max = gtk_entry_get_text(GTK_ENTRY(wg_max));

	wg_read = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_slider_read"));
...
static gboolean isac_validate_text(SPWidget *spw)
{
	...
	wg_tag = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_text_tag"));
	tag = gtk_entry_get_text(GTK_ENTRY(wg_tag));
...
static gboolean isac_validate_tooltips(SPWidget *spw)
{
	...
	wg_size = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_tooltips_size"));
	size = gtk_entry_get_text(GTK_ENTRY(wg_size));

	wg_style = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_tooltips_style"));
...
static gboolean isac_validate_zoom(SPWidget *spw)
{
	...
	wg_tag = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_zoom_tag"));
	tag = gtk_entry_get_text(GTK_ENTRY(wg_tag));

	wg_align = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_zoom_align"));
	align = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(wg_align));    
...
gchar* isac_get_bar_syntax(gchar *syntax, SPWidget *spw)
{
	...
	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_bar_tag"));
	tag = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_bar_min"));
	min = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_bar_max"));
	max = gtk_entry_get_text(GTK_ENTRY(wg));

	gchar *dsb = (gchar*)(g_object_get_data(GTK_OBJECT(spw), "isac_bar_dsb"));
...
gchar* isac_get_clone_syntax(gchar *syntax, SPWidget *spw)
{
	...
	gchar *dsb = (gchar*)(g_object_get_data(GTK_OBJECT(spw), "isac_clone_dsb"));
...
gchar* isac_get_color_syntax(gchar *syntax, SPWidget *spw)
{
	...
	isac_validate_color(spw);
	gchar *dsb = (gchar*)(g_object_get_data(GTK_OBJECT(spw), "isac_color_dsb"));
...
gchar* isac_get_function_syntax(gchar *syntax, SPWidget *spw)
{
	...
	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_func_name"));
	name = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_func_param"));
	...
	gchar *dsb = (gchar*)(g_object_get_data(GTK_OBJECT(spw), "isac_func_dsb"));
...
gchar* isac_get_get_syntax(gchar *syntax, SPWidget *spw)
{
	...
	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_get_tag"));
	tag = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_get_align"));
	align = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_get_type"));
	type = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(wg));
	...
	gchar *dsb = (gchar*)(g_object_get_data(GTK_OBJECT(spw), "isac_get_dsb"));
...
gchar* isac_get_opacity_syntax(gchar *syntax, SPWidget *spw)
{
	...
	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_opac_tag"));
	tag = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_opac_min"));
	min = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_opac_max"));
	max = gtk_entry_get_text(GTK_ENTRY(wg));
	...
	gchar *dsb = (gchar*)(g_object_get_data(GTK_OBJECT(spw), "isac_opac_dsb"));
...
gchar* isac_get_open_syntax(gchar *syntax, SPWidget *spw)
{
	...
	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_open_source"));
	src = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_open_srctype"));
	srctype = gtk_combo_box_get_active(GTK_COMBO_BOX(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_open_type"));
	type = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_open_xpos"));
	x = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_open_ypos"));
	y = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_open_width"));
	width = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_open_height"));
	...
	gchar *dsb = (gchar*)(g_object_get_data(GTK_OBJECT(spw), "isac_open_dsb"));

	if (strcmp(dsb, "0") == 0) o["attr"] = "open";
	else o["@ttr"] = "open";

	if (strcmp(type, "Current window") == 0) o["type"] = "_self";
	else if (strcmp(type, "New window") == 0) o["type"] = "_blank";
	else o["type"] = type;
...
gchar* isac_get_popup_syntax(gchar *syntax, SPWidget *spw)
{
	...
	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_popup_source"));
	src = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_popup_xpos"));
	x = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_popup_ypos"));
	y = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_popup_width"));
	width = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_popup_height"));
	height = gtk_entry_get_text(GTK_ENTRY(wg));
	...
	gchar *dsb = (gchar*)(g_object_get_data(GTK_OBJECT(spw), "isac_popup_dsb"));
...
gchar* isac_get_rotate_syntax(gchar *syntax, SPWidget *spw)
{
	...
	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_rot_tag"));
	tag = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_rot_min"));
	min = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_rot_max"));
	max = gtk_entry_get_text(GTK_ENTRY(wg));
	...
	gchar *dsb = (gchar*)(g_object_get_data(GTK_OBJECT(spw), "isac_rot_dsb"));
...
gchar* isac_get_script_syntax(gchar *syntax, SPWidget *spw)
{
	...
	path = gtk_tree_path_new_first();
	while (gtk_tree_model_get_iter(model, &iter, path)) {
		gtk_tree_model_get(model, &iter, 0, &evt, 2, &scr, -1);
		if (strlen(scr) != 0) {
			oarray["evt"] = evt;
			oarray["param"] = scr;
			ja.push_back(oarray);
	...
	gchar *dsb = (gchar*)(g_object_get_data(GTK_OBJECT(spw), "isac_script_dsb"));
	if (strcmp(dsb, "0") == 0)
		o["attr"] = "script";
	else
		o["@ttr"] = "script";
...
gchar* isac_get_set_syntax(gchar *syntax, SPWidget *spw)
{
	...
	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_set_tag"));
	tag = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_set_align"));
	align = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_set_source"));
	source = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_set_type"));
	type = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_set_prompt"));
	...
	gchar *dsb = (gchar*)(g_object_get_data(GTK_OBJECT(spw), "isac_set_dsb"));
...
gchar* isac_get_slider_syntax(gchar *syntax, SPWidget *spw)
{
	...
	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_slider_tag"));
	tag = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_slider_min"));
	min = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_slider_max"));
	max = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_slider_read"));
	...
	gchar *dsb = (gchar*)(g_object_get_data(GTK_OBJECT(spw), "isac_slider_dsb"));
...
gchar* isac_get_text_syntax(gchar *syntax, SPWidget *spw)
{
	...
	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_text_tag"));
	tag = gtk_entry_get_text(GTK_ENTRY(wg));
	...
	gchar *dsb = (gchar*)(g_object_get_data(GTK_OBJECT(spw), "isac_text_dsb"));
...
gchar* isac_get_tooltips_syntax(gchar *syntax, SPWidget *spw)
{
	...
	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_tooltips_size"));
	size = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_tooltips_style"));
	...
	gchar *dsb = (gchar*)(g_object_get_data(GTK_OBJECT(spw), "isac_tooltips_dsb"));
...
gchar* isac_get_zoom_syntax(gchar *syntax, SPWidget *spw)
{
	...
	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_zoom_tag"));
	tag = gtk_entry_get_text(GTK_ENTRY(wg));

	wg = GTK_WIDGET(g_object_get_data(GTK_OBJECT(spw), "isac_zoom_align"));
	align = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(wg));
	...
	gchar *dsb = (gchar*)(g_object_get_data(GTK_OBJECT(spw), "isac_zoom_dsb"));
...
static void isac_function_assign_syntax(char* token, SPWidget *spw){
	...
	*/
	...
	wg = GTK_WIDGET(g_object_get_data (GTK_OBJECT (spw), "isac_func_effect"));
	...
	case 'o':
		...
		while (gtk_tree_model_get_iter(model, &iter, path)) {
			...
			if (strcmp(evt, event) == 0) {
				...
				scr = jvalue->c_str();
				gtk_list_store_set(store, &iter, 1, PANGO_UNDERLINE_SINGLE, 2, scr, -1);
